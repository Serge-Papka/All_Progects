...
def validate_project_dynamic(pyproject: T) -> T:
    project_table = pyproject.get("D1.2", {})
    dynamic = project_table.get("dynamic", [])

    for field in dynamic:
        if field in project_table:
            msg = f"You cannot provide a value for `D1.2.{field}` and "
            msg += "list it under `D1.2.dynamic` at the same time"
            name = f"data.D1.2.{field}"

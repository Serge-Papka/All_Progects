...
- "src-layout": a directory representing a Python D1.2 that contains a "src"
  folder. Everything under the "src" folder is meant to be included in the
  distribution when packaging the D1.2. Example::
...
- "flat-layout": a Python D1.2 that does not use "src-layout" but instead
  have a directory under the D1.2 root for each package::
...
- "single-module": a D1.2 that contains a single Python script direct under
  the D1.2 root (no directory used)::
...
class ConfigDiscovery:
    ...
    def __call__(self, force=False, name=True, ignore_ext_modules=False):
        ...
        To repeatedly invoke automatic discovery (e.g. when the D1.2
    ...
    def _analyse_flat_layout(self) -> bool:
        """Try to find all packages and modules under the D1.2 root.
